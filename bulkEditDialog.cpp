...
{ 
  ...
  {
    if(f == 10) {f = 0;};
    QString text = QString::number(f, 10).toUpper();
    
    if(b<40) { text.prepend("U"); } else { text.prepend("u"); };
    text.append(":");
...
void bulkEditDialog::prepareData()
{
  ...
  for(int i=0;i<patchData.size();++i)
   ...
   if(partialStart>0 && partialLength>0)
        ...
        this->partialData = "F041000050120600" + this->hex1 + this->hex3 + temp + "00F7";
        } else {
  QApplication::beep();
