    ...
    : stompBox(parent)
{
  ...
  editDetails()->patchPos(246, 14, "03", "00");
	setEditPages();
};
...
void stompbox_fv::setEditPages()
{
  editDetails()->page()->newGroupBox("Foot Volume");
	editDetails()->page()->addKnob(0, 0, 1, 1, "02", "00", "2E");         //foot volume level
	editDetails()->page()->addGroupBox(0, 0, 1, 1);
	
	editDetails()->page()->newGroupBox("Master");
	editDetails()->page()->addKnob(0, 0, 1, 1, "02", "00", "2B");         //Master level
	editDetails()->page()->addGroupBox(0, 2, 1, 2);
	
	editDetails()->page()->newGroupBox("Expression Pedal");
  //editDetails()->page()->addSwitch(0, 0, 1, 1, "00", "00", "00");    //on/off
  editDetails()->page()->addTarget(0, 1, 1, 1, "03", "00", "00", "target", "02", "04");//target
	editDetails()->page()->addGroupBox(1, 0, 1, 4);
	 
	editDetails()->page()->newGroupBox("CTL Pedal");	
	//editDetails()->page()->addSwitch(0, 0, 1, 1, "02", "00", "00");    //on/off
  editDetails()->page()->addTarget(0, 1, 1, 1, "03", "00", "06", "target", "08", "0A");//target
	editDetails()->page()->addComboBox(0, 4, 1, 1, "03", "00", "0D");         // source mode
