                  ...
                  : QWidget(parent)
{
  ...
  ******************************************************/
  ...
  text = "<b>Boss GT-6 Patch Summary</b>    ";
  ...
  QString chain = tr("<br><br><b>Signal Chain =</b> input -> ");
  for(int i= sysxDataOffset;i< (sysxDataOffset + 11);i++ )
  {
     chain.append(" [");
     chain.append( midiTable->getMidiMap("Structure", chainAddress, "00", "00", fxChain.at(i)).name );
     chain.append("]");
  };
  chain.replace("DD", "DELAY");
  chain.replace("CE", "CHORUS");
  chain.replace("RV", "REVERB");
  chain.replace("PRE", "PREAMP/SPKR");
