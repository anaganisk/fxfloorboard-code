    ...
    try {    
		...
		if (!midi){
		 {unsigned int n;
			...
			//sysxEOF.append(sysxOutMsg.mid(i, 2));
			bool ok;
			n = hex.toInt(&ok, 16);
			*ptr = (char)n;
...
void midiIO::sendMidiMsg(QString sysxOutMsg, int midiOutPort)
{
   ...
   /* Clean up*/
 cleanup:
	//SLEEP(30);						// wait as long as the message is sending.
	midiMsgOut->closePort();
    delete midiMsgOut;	
};
...
cleanup:
		...
		dataReceive = true;
		midiin->closePort();             // open the midi in port	
		delete midiin;
};
...
//		emit setStatusMessage(tr("Ready"));
	...
	else   // if not a midi message, then it must be a sysx message
	{
		...
		if(receive==true)
		{
			...
			else if(sizeChunk == "00000D63") // Patch Request data size.
			{
...
{	
  ...
  if (sysxOutMsg.size() == 12){reBuild = sysxOutMsg;};  // identity request not require checksum
	...
	if (deviceType == "GT-10") {
    if(sysxOutMsg.size() == (sysxDataOffset*2 + 12) && sysxOutMsg.mid(sysxOutMsg.size()-8, 4) == "0D63" && sysxOutMsg.mid((sysxAddressOffset*2-2), 2) == "11")  
