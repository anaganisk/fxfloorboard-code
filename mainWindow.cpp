...
void mainWindow::createActions()
{
  ...
  bulkLoadAct = new QAction(QIcon(":/images/fileopen.png"), tr("&Load Bulk Patch File to GT-6..."), this);
	bulkLoadAct->setShortcut(tr("Ctrl+B"));
	bulkLoadAct->setStatusTip(tr("Load Bulk Data to GT-6"));
	connect(bulkLoadAct, SIGNAL(triggered()), this, SLOT(bulkLoad()));

	bulkSaveAct = new QAction(QIcon(":/images/filesave.png"), tr("Save Bulk GT-6 Patches to File..."), this);
  ...
  //SLEEP(3000);
	...
	if (!fileName.isEmpty())	
	{
	  if(!fileName.contains(".GT6_system_syx"))
		{
			fileName.append(".GT6_system_syx");
		};
