...
void mainWindow::createActions()
{
	...
	bulkLoadAct = new QAction(QIcon(":/images/fileopen.png"), tr("&Load Bulk Patch File to GT-5..."), this);
	bulkLoadAct->setShortcut(tr("Ctrl+B"));
	bulkLoadAct->setStatusTip(tr("Load Bulk Data to GT-5"));
	connect(bulkLoadAct, SIGNAL(triggered()), this, SLOT(bulkLoad()));

	bulkSaveAct = new QAction(QIcon(":/images/filesave.png"), tr("Save Bulk GT-5 Patches to File..."), this);
		  ...
		  SysxIO *sysxIO = SysxIO::Instance();
			...
			emit updateSignal();
				QMessageBox *msgBox = new QMessageBox();
					...
					msgText.append(tr("This will overwrite the SYSTEM DATA currently stored in the GT-5<br>"));
					msgText.append(tr (" and can't be undone.<br>"));
					msgText.append(tr("Select 'NO' to only update the Editor settings-<br> Select 'YES' to update the GT-5 memory<br>"));
          

					msgBox->setInformativeText(tr("Are you sure you want to write to the GT-5?"));
