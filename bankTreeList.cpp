...
void bankTreeList::updatePatch(QString replyMsg)
{
   ...
   QFile file(":default.syx");   // Read the default GT-10 sysx file so we don't start empty handed.
    if (file.open(QIODevice::ReadOnly))
	  {	data = file.readAll(); };
	  QByteArray temp;                      
    temp = data.mid(670, 228);           // copy patch description from default.syx  address 00 0D 00 00   
	...
	QString msgText;
	msgText.append("<font size='+1'><b>");
	msgText.append(QObject::tr("Please make sure the editor and ") + deviceType + ("<br> are set to 'Bulk Mode' and re-try."));
	msgBox->setText(msgText);
	msgBox->setStandardButtons(QMessageBox::Ok);
	msgBox->exec();
	/* END WARNING */
