...
void bankTreeList::updatePatch(QString replyMsg)
{
   ...
   QFile file(":default.syx");   // Read the default GT-5 sysx file so we don't start empty handed.
    if (file.open(QIODevice::ReadOnly))
	  {	data = file.readAll(); };
	  QByteArray temp;                      
    temp = data.mid(426, 228);           // copy patch description from default.syx  address 00 16 00 00
	...
	replyMsg.append(sysxBuffer);
		...
		sysxIO->setFileName(tr("Patch from ") + deviceType);	// Set the file name to GT-5 patch for the display.
	...
	msgBox->setWindowTitle(QObject::tr("Warning - Patch data received is incomplete!"));
