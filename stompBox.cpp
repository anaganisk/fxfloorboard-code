...
void stompBox::emitValueChanged(QString hex1, QString hex2, QString hex3, QString valueHex)
{
	...
	else
	{
		...
		if(hex1 == "03" && hex3 >= "0B") // Rename the Pre amp to Speaker Cabinet, shared memory location
			{
				this->fxName = "Speaker Cabinet";
			}
		if(hex1 == "03" && hex3 <= "0A" && hex3 >= "01") // Rename the Pre amp to Speaker Cabinet, shared memory location
			{
