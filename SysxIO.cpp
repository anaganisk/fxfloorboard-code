...
void SysxIO::setFileSource(QByteArray data)
{
	...
	for(int i=0;i<data.size();i++)
	{
		...
		if(offset > 5 && nexthex != "F7")                              // changed from > 6 ... not work with 6
		{		
			dataSize += n;
		};
		if(nexthex == "F7")
		{		
			...
			if(getCheckSum(dataSize) != checksum)
			{
				...
				errorString.append(sysxBuffer.at(6) + " ");     // reduced by 1 ... GT6B string 1 byte shorter
				errorString.append(sysxBuffer.at(7) + " ");     // reduced by 1
				errorString.append(sysxBuffer.at(8) + " ");     // reduced by 1
				errorString.append(sysxBuffer.at(9) + " - ");   // reduced by 1
		...
		if(hex == "F7") 
		{	
			this->fileSource.address.append( sysxBuffer.at(8) + sysxBuffer.at(9) );
...
{
	bool ok;
	int dataOffset = 10;   // changed from 11
...
{
	QString address1 = sysxMsg.at(8);
	QString address2 = sysxMsg.at(9); 

	bool ok;

	MidiTable *midiTable = MidiTable::Instance();
	for(int i=10;i<sysxMsg.size() - 3;i++)
	{
		if(i==12) i++;
