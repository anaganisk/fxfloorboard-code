...
void SysxIO::receiveSysx(QString sysxMsg)
{
	...
	if (sysxMsg.size() > 0){
			...
			snork.append("}");	
			snork.append(tr("<br> midi data received<br>"));
			if (sysxMsg == dBug){
				  snork.append(tr(" WARNING: midi data received = data sent"));
				  snork.append(tr("<br> caused by a midi loopback, port change is required<br>"));
			 };
	     ...
	     if (isConnected())
	       {
	        emit setStatusSymbol(2);
		      ...
		      sendSysx(systemRequest); // GT-6 System area data Request.    
...
void SysxIO::writeToBuffer() 
{
	...
	QString addr1 = "0B"; //QString::number(96, 16).toUpper();  // temp address
