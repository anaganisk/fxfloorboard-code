...
void SysxIO::setFileSource(QByteArray data)
{
	...
	/*if(!errorList.isEmpty())
	{
		...
		//msgBox->setWindowTitle(tr("GT-Pro Fx FloorBoard - Checksum Error"));
		msgBox->setWindowTitle(tr("GT-Pro Fx FloorBoard"));
		...
		msgText.append(tr("The incorrect values have been corrected where possible.\n" 
			"If correction was impossible then bad settings will have been set to midway."));
...
QList<QString> SysxIO::correctSysxMsg(QList<QString> sysxMsg)
{
	...
	{
		if(i==sysxDataOffset + 1) i++; // is reserved memmory address on the GT-Pro so we skip it.
