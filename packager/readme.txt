...
e.) A single mouse pointer click on the Patch-Tree name will change the GT10 to that selected patch only - sound played through the GT10 will have the sound of that patch.
 
f.) A double click on the patch name will load the patch information from the Gt10 into the editor, all the patch settings will update the editor GUI.

g.) Any knob turning will change the sound of the effect being adjusted, when the desired sound is achieved, you can either save the patch to the GT10 or save the patch to file on disk.
