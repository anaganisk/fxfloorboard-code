...
void floorBoard::dropEvent(QDropEvent *event)
{
    if (event->mimeData()->hasFormat("application/x-stompbox")) 
	{
      ...
      SysxIO *sysxIO = SysxIO::Instance();
			...
			sysxIO->setFileSource("Structure", "0A", "00", "00", hexData);
			};
		}
		else
		{
...
void floorBoard::initSize(QSize floorSize)
{
	...
	//for(int i=10;i>=0;i--)
	{
		...
		if(i>5)
		{
			...
			x = x - (( stompSize.width() + spacingH ) * 5.5);
		};
...
void floorBoard::initStomps()
{
	...
	/*   FX   WAH  OD   PRE  EQ   FX2  DD   CE   RV    NS    FV :depend on midi.xml   */
	this->fx = fx;

	QVector<QString> initStompNames(11);
	this->stompNames = initStompNames.toList();;

	QVector<stompBox *> initStompBoxes(11);
...
{
	SysxIO *sysxIO = SysxIO::Instance();
	QList<QString> fxChain = sysxIO->getFileSource("Structure", "0A", "00");
