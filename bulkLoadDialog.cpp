...
void bulkLoadDialog::comboValueChanged(int value)
{
  ...
  if (x<0) {x=0; } else if (x>139) { x=139; bankStart=139-(finishList-startList);  startRangeComboBox->setCurrentIndex(139-(finishList-startList)); };
  ...
  this->startRangeComboBox->setMaxVisibleItems(140-(finishList-startList));
}; 
...
{	
  ...
  progress = 0;
  patch = 1;
  range = 140/(finishList-startList+1);
   ...
   for (int a=startList;a<(finishList+1);a++)
   {
      ...
      int b = a*1010;                                // multiples of patch size.
       msg = this->bulk.mid(b*2, 1010*2);             // copy next patch from bulk patch list.
...
{ 
  ...
  sysxIO->setDeviceReady(true); // Free the device after finishing interaction.
     msg=bulk.mid(steps*1010*2, 1010*2);    
...
{
  patchCount = sysxPatches.size()/1010;
  QString msgText;
  QString patchText;
	QString patchNumber;
	unsigned char r;
