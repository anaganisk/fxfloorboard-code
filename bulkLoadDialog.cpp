...
{ 
  ...
  Preferences *preferences = Preferences::Instance();
	...
	QString fileName = QFileDialog::getOpenFileName(
                ...
                tr("Choose a file"),
                dir,
                tr("GT10 Bulk Data File (*.gxg *.syx *.mid)"));
	if (!fileName.isEmpty())	
	{
    ...
    if (file.open(QIODevice::ReadOnly))
	  ...
	  QFile GXGfile(":default.gxg");           // Read the default GT-10B GXG file .
...
{	
  ...
  progress = 0;
  patch = 1;
  range = 200/(finishList-startList+1);
   ...
   for (int a=startList;a<(finishList+1);a++)
   {
      ...
      int b = a*2045;                                // multiples of patch size.
       msg = this->bulk.mid(b*2, 2045*2);             // copy next patch from bulk patch list.
  ...
  if (sysxEOF == "F7")
	...
	bank = (bankStart+3/*+(finishList-startList)/4*/);
  patch = bank/4; patch = patch*4; patch=bank-patch;
          steps=0;
          dataSent=0;
          sendSequence("");
};	
...
{ 
  ...
  sysxIO->setDeviceReady(true); // Free the device after finishing interaction.
     msg=bulk.mid(steps*2045*2, 2045*2);    
  ...
  {      
        bool ok;
        QString patchText;
        QString name = msg.mid(22, 32);                       // get name from loaded patch. 
        QList<QString> x;        
        for (int b=0;b<16;b++)
        {
...
{
  patchCount = sysxPatches.size()/2045;
  QString msgText;
  QString patchText;
	QString patchNumber;
	unsigned char r;
        ...
        for (int b=0;b<16;b++)
           {
            ...
            patchText.clear();
            msgText.clear();
            a=a+2045;                      // offset is set in front of marker
        }; 
...
{	
  ...
  QMessageBox *msgBox = new QMessageBox();
	...
	msgText.append(QObject::tr("This is not a GT-10 file!"));
     ...
     for (int h=0;h<count;h++)
       {                
          temp = data.mid(a+43, 128);            // copy SMF 128 bytes
        	...
        	default_data.replace(434, 128, temp);            // replace gt10 address "03"
        	temp = data.mid(a+587, 128);           // copy SMF part1
        	default_data.replace(575, 128, temp);            // replace gt10 address "04"
        	temp = data.mid(a+715, 86);            // copy SMF part1
        	default_data.replace(716, 86, temp);             // replace gt10 address "05"
        	temp = data.mid(a+859, 128);           // copy SMF part1
        	default_data.replace(815,128, temp);             // replace gt10 address "06"
        	temp = data.mid(a+987, 72);            // copy SMF part1
        	temp.append(data.mid(a+1075,56));      // copy SMF part2
        	default_data.replace(956, 128, temp);            // replace gt10 address "07"
        	temp = data.mid(a+1131, 128);          // copy SMF part1
        	default_data.replace(1097,128, temp);            // replace gt10 address "08"
        	temp = data.mid(a+1259, 58);           // copy SMF part1
        	temp.append(data.mid(a+1333,42));      // copy SMF part2
        	default_data.replace(1238, 100, temp);           // replace gt10 address "09"
        	temp = data.mid(a+1403, 128);          // copy SMF part1
        	default_data.replace(1351,128, temp);            // replace gt10 address "0A"
        	temp = data.mid(a+1531, 44);           // copy SMF part1
        	temp.append(data.mid(a+1591,84));      // copy SMF part2
        	default_data.replace(1492, 128, temp);           // replace gt10 address "0B"
        	temp = data.mid(a+1675, 128);          // copy SMF part1
        	default_data.replace(1633,128, temp);            // replace gt10 address "0C"        
