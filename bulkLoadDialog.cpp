...
{ 
  ...
  Preferences *preferences = Preferences::Instance();
	...
	QString fileName = QFileDialog::getOpenFileName(
                ...
                tr("Choose a file"),
                dir,
                tr("GTPro Bulk Data File (*.gte *.syx *.mid)"));
	if (!fileName.isEmpty())	
	{
    ...
    if (file.open(QIODevice::ReadOnly))
	{
		...
		QFile file(":default.syx");           // Read the default GT-Pro sysx file .
    if (file.open(QIODevice::ReadOnly))
    ...
    if (hexfile.open(QIODevice::ReadOnly))
	  ...
	  QByteArray GTE_header = GTE_default.mid(0, 23);               // copy header from default.gte
...
void bulkLoadDialog::updatePatch()
{
  ...
  unsigned int a = sysxNameOffset;             // locate patch text start position from the start of the file
   ...
   this->finishRange->setText(text);   
    ...
    this->startRangeComboBox->addItem(bnk);    
    bnk = "U" + QString::number(x+1, 10).toUpper() + "-5";
    this->startRangeComboBox->addItem(bnk);
    bnk = "U" + QString::number(x+1, 10).toUpper() + "-6";
    this->startRangeComboBox->addItem(bnk);
    bnk = "U" + QString::number(x+1, 10).toUpper() + "-7";
    this->startRangeComboBox->addItem(bnk);
    bnk = "U" + QString::number(x+1, 10).toUpper() + "-8";
    this->startRangeComboBox->addItem(bnk); 
    bnk = "U" + QString::number(x+1, 10).toUpper() + "-9";
    this->startRangeComboBox->addItem(bnk);
    bnk = "U" + QString::number(x+1, 10).toUpper() + "-10";
    this->startRangeComboBox->addItem(bnk);     
  }; 
