...
{	
    ...
    if (file.open(QIODevice::ReadOnly))
	  ...
	  QByteArray default_header = default_data.mid(0, 6);           // copy header from default.syx
          QByteArray text_header = default_data.mid(patchSize, 6);            // copy header from default.syx from text area
	  ...
	  if (n < 6){ isSystem = true; };
                ...
                if(data.size() == (patchSize+228) && isHeader && !isSystem){
		...
		return true;
                ...
                else if (data.size() == patchSize && isHeader && !isSystem)
    {
    QByteArray standard_data = data;
	  QFile file(":default.syx");   // Read the default GT-5 sysx file so we don't start empty handed.
    if (file.open(QIODevice::ReadOnly))
	  {	data = file.readAll(); };
	  
	  QByteArray temp;                      
    temp = data.mid(patchSize, 228);           // copy patch description from default.syx  address 00 17 00 00
  ...
  if (index>0)
   { 
    ...
    temp = patch_data.mid(4206, 650);                    
    temp.append(data.mid(patchSize, 228));           // copy patch description from default.syx  address 00 17 00 00
     };
    data = temp;
