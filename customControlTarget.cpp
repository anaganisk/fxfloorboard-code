	...
	: QWidget(parent)
{
	...
	if(valueHex.length() < 2) valueHex.prepend("0");
        ...
        //if(valueStr.length() < 2) valueStr.prepend("0");

        Midi items = midiTable->getMidiMap("Structure", hex1, hex2, hex3, valueHex);
  ...
  QObject::connect(this->displayCombo, SIGNAL( updateSignal() ),
                this, SLOT( TargetKnobUpdateSignal() ));
...
void customControlTarget::TargetKnobUpdateSignal()
{
    SysxIO *sysxIO = SysxIO::Instance();
