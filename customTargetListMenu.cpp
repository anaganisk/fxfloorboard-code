...
customTargetListMenu::customTargetListMenu(QWidget *parent, QString hex1, QString hex2,
                                           QString hex3, QString hexMsb, QString hexLsb,
...
{    
	...
	{
		QString item;
                QString listItem = items.level.at(itemcount).name;
                if(!listItem.isEmpty())
		{
                        item = listItem;
		}
...
{   
	...
	if(valueHex.length() < 2) valueHex.prepend("0");  
        ...
        Midi items = midiTable->getMidiMap("Structure", hex1, hex2, hex3, valueHex);//hex4, hex5);
