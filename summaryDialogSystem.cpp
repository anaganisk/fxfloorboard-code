                  ...
                  : QWidget(parent)
{
  ...
  textDialog->setDocumentTitle(deviceType + tr(" System data List"));
  textDialog->setMinimumHeight(600);
         ...
         {
              QString snork = tr("Ensure 'Bulk Mode' is active and retry<br>");
              ...
              msgBox->exec();
              emit setStatusMessage(tr("No Bulk Mode connection"));
        ...
        if(replyMsg.size()/2 == 4206)
          {
                ...
                system = replyMsg;
                sysxIO->setFileName(tr("System Data from ") + deviceType);	// Set the file name to GT-6B system for the display.
...
void summaryDialogSystem::generatePage()
{
  ...
  text.append("<br><br><b><u>**********Manual Mode Pedals**********</b></u><br>");
  address1= "03";
  address2= "00";
  address3= "00";
  start = 0;
    ...
    for(int i=0;i<64;i++ )
      {
        ...
        QString pos = QString::number(i, 16).toUpper();
        if(pos.size()<2){ pos.prepend("0"); };
        QString txt =  QString::number(count, 10);
        if (txt.size()<2) {txt.prepend("00"); };
        if (txt.size()<3) {txt.prepend("0"); };
        txt.prepend(" PC#");
        txt.prepend(QString::number(bankCount, 10));
        txt.prepend("[Bank#");
        txt.append("] = ");
          ...
          text.append(txt);         
          text.append(midiTable->getValue("System02", "02", "10", "00", valueHex) );
          text.append(".    ."); 
          ++row;
          if (row >= 5) {row = 1; text.append("<br>"); };       
         ++count;
          if(count > 128) {count = 1; bankCount += 1; };
        ++i;
      };   
};
